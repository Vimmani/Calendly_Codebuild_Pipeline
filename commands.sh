...
DOCKER_BUILDKIT=0 docker build -t calendly .
...
aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 337964480109.dkr.ecr.us-east-1.amazonaws.com
...
# Adding Comment to Test continous deployment using AWS codebuild
